...
def _serve_robot_from_cfg(robot_cfg):
    from gello.utils.launch_utils import instantiate_from_dict
...
def main():
    ...
    robot_target = left_robot_cfg.get("_target_", "")
    is_mujoco = robot_target.endswith("MujocoRobotServer")
    ...
    if is_mujoco:
        ...
        server_process = Process(target=_serve_robot_from_cfg, args=(left_robot_cfg,))
        server_process.start()
        active_servers.append(server_process)
        ...
        robot_name = "MujocoRobotServer"

    elif robot is not None and hasattr(robot, "serve"):  # MujocoRobotServer or ZMQServerRobot
        print("Starting robot server...")
        from gello.env import RobotEnv
        from gello.zmq_core.robot_node import ZMQClientRobot
        ...
        # Start server in background (prefer process for MuJoCo viewer stability)
        ...
        if robot is None:
            robot = instantiate_from_dict(left_robot_cfg)
    ...
    print(f"Launching robot: {robot_name}, agent: {agent.__class__.__name__}")
