...
def attach_hand_to_arm(
    ...
    # Rotate the gripper to match GELLO orientation (see gello_control setup).
    hand_body = hand_mjcf.find("body", "hand")
    if hand_body is not None:
        try:
            if hasattr(hand_body, "quat") and hand_body.quat is not None:
                current_quat_list = list(hand_body.quat)
                if len(current_quat_list) == 4:
                    # mjcf may return [x, y, z, w]; convert to [w, x, y, z]
        ...
        angle_rad = -135 * np.pi / 180.0
        rotation_quat = np.array(
            [np.cos(angle_rad / 2.0), np.sin(angle_rad / 2.0), 0.0, 0.0]
        )
