...
struct RecordedData {
  std::vector<double> time;
  std::vector<std::array<double, 7>> q_actual;
  std::vector<std::array<double, 7>> q_desired;
  std::vector<double> gripper_width;
};
...
void saveRecordedData(const RecordedData& data, const std::string& path) {
  ...
  for (size_t i = 0; i < data.time.size(); ++i) {
    file << "    {\n";
    file << "      \"timestamp\": " << data.time[i] << ",\n";

    file << "      \"q_actual\": [";
    for (size_t j = 0; j < 7; ++j) {
      file << data.q_actual[i][j];
      if (j < 6) file << ", ";
    }
    file << "],\n";
    ...
    file << "      \"q_desired\": [";
    for (size_t j = 0; j < 7; ++j) {
      file << data.q_desired[i][j];
      if (j < 6) file << ", ";
    }
    file << "],\n";

    file << "      \"gripper_width\": " << data.gripper_width[i] << "\n";

    file << "    }";
    if (i < data.time.size() - 1) {
      file << ",";
    }
    file << "\n";
  }
  ...
  std::cout << "Saved recorded data to: " << path << std::endl;
}
...
// Gripper control thread function with recording
void gripperControlThread(const std::string& robot_ip,
                          ...
                          std::atomic<double>& shared_gripper_width,
  ...
  try {
    ...
    while (!arm_control_finished.load() && !gripper_should_stop.load()) {
      ...
      // Update shared gripper width
      try {
        auto state = gripper.readOnce();
        shared_gripper_width.store(state.width);
      } catch (...) {
        // Ignore read errors, keep last value
      }
...
int main(int argc, char** argv) {
  if (argc < 3) {
    std::cerr << "Usage: " << argv[0]
              << " <robot_ip> <json_path> [time_scale] [start_speed] [k] [d] [k_alpha] [q_alpha]"
                 " [gripper_speed] [gripper_force] [output_path]\n";
    return 1;
  }
  ...
  std::string output_path = (argc >= 12) ? argv[11] : "/home/rsj/franka_cpp_control/recorded_trajectory.json";
  ...
  try {
    ...
    std::atomic<double> shared_gripper_width{0.0};
    ...
    recorded_data.time.reserve(20000);
    recorded_data.q_actual.reserve(20000);
    recorded_data.q_desired.reserve(20000);
    recorded_data.gripper_width.reserve(20000);
    ...
    const int record_decimation = 10;  // Record every 10ms (10 control cycles at 1kHz)
    ...
    auto control_callback = [&](const franka::RobotState& state,
      ...
      // Record data (decimated)
      if (record_counter % record_decimation == 0) {
        recorded_data.time.push_back(time);
        std::array<double, 7> q_actual;
        for (size_t i = 0; i < 7; ++i) {
          q_actual[i] = state.q[i];
        }
        recorded_data.q_actual.push_back(q_actual);
        recorded_data.q_desired.push_back(q_goal_filtered);
        recorded_data.gripper_width.push_back(shared_gripper_width.load());
      }
      record_counter++;
    ...
    // Save recorded data
    saveRecordedData(recorded_data, output_path);

    std::cout << "All controls finished successfully." << std::endl;
    std::cout << "Recorded " << recorded_data.time.size() << " data points." << std::endl;
