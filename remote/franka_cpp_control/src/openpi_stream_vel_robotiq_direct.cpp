...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cerr << "Usage: " << argv[0]
              << " <robot_ip> [action_port] [state_port] [rate_hz] [max_acc] [vel_alpha] [timeout_s] [invert_gripper] [kp]"
              << "\n";
    return 1;
  }
  ...
  const double kp = (argc >= 10) ? std::stod(argv[9]) : 0.0;
  ...
  try {
    ...
    auto control_callback = [&](const franka::RobotState& state,
      ...
      if (!q_des_initialized) {
        q_des = state.q_d;
        q_des_initialized = true;
      }
      ...
      while (accum >= action_dt) {
        ...
        for (size_t i = 0; i < 7; ++i) {
          q_des[i] += current_action[i] * action_dt;
        }
        updated = true;
      }
      ...
      for (size_t i = 0; i < 7; ++i) {
        const double dq_raw = current_action[i];
