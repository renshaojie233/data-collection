...
bool envArray(const char* name, size_t count, std::vector<double>& out) {
  const char* value = std::getenv(name);
  if (!value || !value[0]) {
    return false;
  }
  std::vector<double> parsed;
  if (!parseDoubleList(value, parsed)) {
    return false;
  }
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cerr << "Usage: " << argv[0]
              << " <robot_ip> [action_port] [state_port] [rate_hz] [k] [d] [k_alpha]"
                 " [q_alpha] [timeout_s]\n";
    return 1;
  }
  ...
  const double rate_hz = (argc >= 5) ? std::stod(argv[4]) : 30.0;
  ...
  const double timeout_s = (argc >= 10) ? std::stod(argv[9]) : 2.0;
  const bool interp_actions = envFlag("ACTION_INTERP", false);
  ...
  d_gains.fill(d_gain);
  {
    ...
    if (envArray("K_GAINS", 7, k_env)) {
      for (size_t i = 0; i < 7; ++i) {
        k_gains[i] = k_env[i];
      }
    }
    std::vector<double> d_env;
    if (envArray("D_GAINS", 7, d_env)) {
      for (size_t i = 0; i < 7; ++i) {
        d_gains[i] = d_env[i];
      }
    }
  ...
  if (interp_actions) {
    std::cerr << "Action interpolation enabled." << std::endl;
  }

  ActionReceiver receiver(action_port, action_buffer, &action_logger);
  receiver.start();

  StateServer state_server(state_port, state_cache, rate_hz);
  state_server.start();
  ...
  std::thread gripper_thread([&]() {
    ...
    const bool invert_gripper = envFlag("INVERT_GRIPPER", false);
  ...
  try {
    ...
    const bool use_gravity_comp = envFlag("USE_GRAVITY_COMP", false);
    const double load_mass = envDouble("LOAD_MASS", -1.0);
    const double load_scale = envDouble("LOAD_SCALE", 1.0);
    std::array<double, 3> load_com{};
    std::array<double, 9> load_inertia{};
    if (load_mass > 0.0) {
      std::vector<double> com_vec;
      std::vector<double> inertia_vec;
      if (!envArray("LOAD_COM", 3, com_vec) || !envArray("LOAD_INERTIA", 9, inertia_vec)) {
        throw std::runtime_error("LOAD_COM or LOAD_INERTIA missing/invalid");
      }
      for (size_t i = 0; i < 3; ++i) {
        load_com[i] = com_vec[i];
      }
      for (size_t i = 0; i < 9; ++i) {
        load_inertia[i] = inertia_vec[i];
      }
      double scaled_mass = load_mass;
      if (load_scale > 0.0) {
        scaled_mass *= load_scale;
        for (double& v : load_inertia) {
          v *= load_scale;
        }
      }
      robot.setLoad(scaled_mass, load_com, load_inertia);
    }

    auto model = robot.loadModel();

    const double action_dt = 1.0 / std::max(1e-6, rate_hz);
    action_buffer.sample_dt = action_dt;
    std::array<double, 8> current_action{};
    std::array<double, 8> next_action{};
    ...
    bool have_next_action = false;
    double step_accum = 0.0;

    auto pop_action = [&](std::array<double, 8>& out) -> bool {
      std::array<double, 8> raw{};
      bool got = false;
      {
        std::lock_guard<std::mutex> lock(action_buffer.mutex);
        if (!action_buffer.samples.empty()) {
          raw = action_buffer.samples.front().action;
      ...
      std::array<double, 8> mapped = raw;
      for (size_t i = 0; i < 7; ++i) {
        const int src = action_map[i];
        if (src >= 0 && src < 7) {
          mapped[i] = raw[static_cast<size_t>(src)];
        }
      }
      out = mapped;
      return true;
    ...
    auto control_callback = [&](const franka::RobotState& state,
      ...
      if (interp_actions) {
        while (step_accum >= action_dt) {
          step_accum -= action_dt;
          if (!have_action) {
            if (pop_action(current_action)) {
              have_action = true;
              gripper_target.store(current_action[7]);
              action_logger.logAction("exec", current_action);
            }
          } else if (have_next_action) {
            current_action = next_action;
            have_next_action = false;
            gripper_target.store(current_action[7]);
            action_logger.logAction("exec", current_action);
          } else {
            if (pop_action(current_action)) {
        ...
        if (have_action && !have_next_action) {
          if (pop_action(next_action)) {
            have_next_action = true;
          }
        }
      } else {
        while (step_accum >= action_dt) {
        ...
        if (action_buffer.has_start_time) {
          const auto elapsed =
              std::chrono::duration<double>(std::chrono::steady_clock::now() -
                                            action_buffer.last_update)
                  .count();
          stale = elapsed > timeout_s;
        }
      }

      std::array<double, 7> q_target = state.q;
      if (have_action && !stale) {
        std::array<double, 8> action_interp{};
        const std::array<double, 8>* action_ptr = &current_action;
        if (interp_actions && have_next_action) {
          const double alpha = clampDouble(step_accum / action_dt, 0.0, 1.0);
          for (size_t i = 0; i < action_interp.size(); ++i) {
            action_interp[i] =
                (1.0 - alpha) * current_action[i] + alpha * next_action[i];
          }
          action_ptr = &action_interp;
        }
        for (size_t i = 0; i < 7; ++i) {
          q_target[i] = (*action_ptr)[i];
        }
        gripper_target.store((*action_ptr)[7]);
      }

      for (size_t i = 0; i < 7; ++i) {
        q_goal_filtered[i] =
            (1.0 - q_alpha) * q_goal_filtered[i] + q_alpha * q_target[i];
      }
      ...
      const auto gravity = use_gravity_comp ? model.gravity(state)
                                            : std::array<double, 7>{};
      std::array<double, 7> tau_d{};
      for (size_t i = 0; i < 7; ++i) {
        tau_d[i] = coriolis[i] + gravity[i] +
                   k_gains[i] * (q_goal_filtered[i] - state.q[i]) +
    ...
    std::cout << "Waiting for position stream on port " << action_port
