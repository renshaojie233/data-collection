...
Trajectory loadTrajectory(const std::string& path) {
  ...
  for (const auto& entry : root.get_child("data")) {
    ...
    for (size_t i = 0; i < 7; ++i) {
      dq[i] = vel[static_cast<size_t>(index[i])];
    }
...
void interpolate(const Trajectory& traj, double t, std::array<double, 7>& dq_out) {
  if (t <= 0.0) {
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    auto control_callback = [&](const franka::RobotState& state,
      ...
      interpolate(traj, t_traj, dq_des);
    ...
    std::cout << "Tracking trajectory with velocities only (" << duration << " s)..."
              << std::endl;
