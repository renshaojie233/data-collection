...
class ActionReceiver {
 public:
  ActionReceiver(int port, ActionBuffer& buffer) : port_(port), buffer_(buffer) {}
    ...
    while (g_running.load()) {
      ...
      while (g_running.load()) {
        ...
        while ((pos = buffer.find('\n')) != std::string::npos) {
          ...
          buffer_.chunk.actions = std::move(actions);
          buffer_.chunk.index = 0;
          buffer_.chunk.last_update = std::chrono::steady_clock::now();
        }
      }
      ::close(client_fd);
    }
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cerr << "Usage: " << argv[0]
              << " <robot_ip> [action_port] [state_port] [rate_hz] [vel_scale] [max_acc] [vel_alpha] [timeout_s] [invert_gripper]"
              << "\n";
    return 1;
  }
  ...
  const double vel_scale = (argc >= 6) ? std::stod(argv[5]) : 0.2;
  const double max_acc = (argc >= 7) ? std::stod(argv[6]) : 5.0;
  const double vel_alpha = (argc >= 8) ? std::stod(argv[7]) : 0.9;
  const double timeout_s = (argc >= 9) ? std::stod(argv[8]) : 2.0;
  const bool invert_gripper = (argc >= 10) ? (std::stoi(argv[9]) != 0) : false;
  ...
  ActionReceiver receiver(action_port, action_buffer);
  receiver.start();
    ...
    auto control_callback = [&](const franka::RobotState& state,
      ...
      while (accum >= action_dt) {
        ...
        const auto elapsed = std::chrono::duration_cast<std::chrono::duration<double>>(
            now - action_buffer.chunk.last_update);
        if (action_buffer.chunk.actions.empty() || elapsed.count() > timeout_s) {
          current_action.fill(0.0);
        } else if (action_buffer.chunk.index < action_buffer.chunk.actions.size()) {
          current_action = action_buffer.chunk.actions[action_buffer.chunk.index];
          action_buffer.chunk.index += 1;
        } else {
          current_action.fill(0.0);
        }
        updated = true;
      ...
      for (size_t i = 0; i < 7; ++i) {
        const double v = clampDouble(current_action[i], -1.0, 1.0);
        dq_cmd[i] = v * vel_scale;
      }

      const auto upper = franka::computeUpperLimitsJointVelocity(state.q);
      const auto lower = franka::computeLowerLimitsJointVelocity(state.q);
      ...
      if (++debug_counter % 1000 == 0) {
        std::cerr << "debug action0=" << current_action[0]
                  << ", dq_cmd0=" << dq_cmd[0]
                  << ", dq_limited0=" << dq_limited[0]
                  << std::endl;
      }
