...
int main(int argc, char** argv) {
  if (argc < 3) {
    std::cerr << "Usage: " << argv[0]
              << " <robot_ip> <json_path> [time_scale] [start_speed] [max_vel] [max_acc] [vel_alpha]\n";
    return 1;
  }
  ...
  try {
    ...
    auto control_callback = [&](const franka::RobotState& state,
      ...
      for (size_t i = 0; i < 7; ++i) {
        dq_cmd[i] = (q_des[i] - last_q_des[i]) / ctrl_dt;
      }
      last_q_des = q_des;
    ...
    std::cout << "Tracking trajectory with velocities (raw, " << duration << " s)..."
              << std::endl;
